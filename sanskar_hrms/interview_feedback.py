...
def get_applicable_interviewers(interview: str) -> list[str]:
    interviewers = frappe.get_all("Interview Detail", filters={"parent": interview}, pluck="interviewer")
    
    # Add HR managers to the list of interviewers
    hr_managers = frappe.get_all("User", filters={"role": "HR Manager"}, pluck="name")
    interviewers.extend(hr_managers)
